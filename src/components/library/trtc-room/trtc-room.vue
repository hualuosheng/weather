    ...
    <view class="trtc-room-container">
      <view v-if="template === '1v1'">
        <view
          data-type="template"
          data-is="1v1"
          data-attr="pusher, streamList, debug"
        >
          <view class="template-1v1">
            <view
              ...
              :class="
                'view-container player-container ' +
                  (item.isVisible ? '' : 'none')
              "
            >
              <live-player
            ...
            <view
              :class="
                'view-container pusher-container ' +
                  (pusher.isVisible ? '' : 'none') +
                  ' ' +
                  (JSON.stringify(streamList) == '[]' ? 'fullscreen' : '')
              "
            >
              <live-pusher
                class="pusher"
              ...
              <view class="loading" v-if="streamList.length === 0">
                <view class="loading-img"
                  ><image
                    src="../../static/components/trtc-room/static/loading.png"
                    class="rotate-img"
                  ></image
                ></view>
        ...
        <image src="{{pusher.enableCamera? '../../static/components/trtc-room/static/camera-true.png': '../../static/components/trtc-room/static/camera-false.png'}} "></image>
            ...
            <view class="bottom-btns">
              <view class="btn-hangup" @tap="hangUpFun"
                ><image
                  src="../../static/components/trtc-room/static/hangup.png"
                ></image
              ></view>
            </view>
          </view>
      ...
      <view v-if="template === 'grid'">
        <view
          data-type="template"
          data-is="grid"
          data-attr="pusher, streamList, debug, panelName"
        >
          <view
            :class="
              'template-grid ' +
          ...
          >
            ...
            >
              ...
              <view class="operation-bar">
                <view
                  class="btn-normal"
                  @tap="handleSubscribeRemoteAudioFun"
                ...
                <view
                  class="btn-normal"
                  @tap="handleSubscribeRemoteVideoFun"
                  :data-user-i-d="item.userID"
                  :data-stream-type="item.streamType"
                >
                  <image
                ...
                <view
                  class="btn-normal"
                  @tap="toggleFullscreenFun"
                  :data-user-i-d="item.userID"
                  :data-stream-type="item.streamType"
                >
                  <image
                    src="../../static/components/trtc-room/static/fullscreen.png"
                  ></image>
                </view>
              </view>
              <progress
            ...
            >
              ...
              ></live-pusher>
              <view class="operation-bar">
                <view class="btn-normal" @tap="switchMemberListPanelFun"
                  ><image
                    src="../../static/components/trtc-room/static/list.png"
                  ></image
                ></view>
                <view class="btn-normal" @tap="switchSettingPanelFun"
                  ><image
                    src="../../static/components/trtc-room/static/setting.png"
                  ></image
                ></view>
                <view class="btn-normal btn-hangup" @tap="hangUpFun"
                  ><image
                    src="../../static/components/trtc-room/static/hangup.png"
                  ></image
                ></view>
                ...
                <scroll-view class="scroll-container" scroll-y="true">
                  ...
                  <view class="setting-option">
                    ...
                    <switch
                      color="#006eff"
                      :checked="
                        pusher.videoOrientation === 'vertical' ? false : true
                      "
            ...
            <view
              :class="'masker ' + (panelName == '' ? 'none' : '')"
              @tap="handleMaskerClickFun"
            ></view>
          </view>
        </view>
      </view>
      ...
      <view
        ...
        ><button @tap="debugTogglePanelFun" hover-class="button-hover">
          Debug
        </button></view
      >
      <view :class="'debug-info ' + (debugMode && debugPanel ? '' : 'none')">
        ...
        <view>
          camera:
          <button
            :class="pusher.enableCamera ? '' : 'false'"
            @tap="toggleVideoFun"
            hover-class="button-hover"
          >
            {{ pusher.enableCamera }}
        ...
        <view>
          switch camera:
          <button @tap="switchCamera" hover-class="button-hover">
            {{ cameraPosition || pusher.frontCamera }}
          </button>
        </view>
        <view>
          Room:
          <button @tap="debugEnterRoomFun" hover-class="button-hover">
            Enter
          </button>
          <button @tap="debugExitRoomFun" hover-class="button-hover">
            Exit
          </button>
          <button @tap="debugGoBackFun" hover-class="button-hover">
            Go back
          </button>
        </view>
        <view>user count: {{ userList.length }}</view>
        <view v-for="(item, userID) in userList" :key="userID">
          {{ item.userID }}|mainV:{{ item.hasMainVideo || false }}|mainA:{{
            item.hasMainAudio || false
          }}|auxV:{{ item.hasAuxVideo || false }}
        </view>
...
<script>
import { setData } from './debug/debug'
  ...
  methods: {
    ...
    setViewRect: function(params) {
      ...
      if (this.pusher.template !== 'custom') {
        console.warn(
          `如需使用setViewRect方法，请设置template:"custom", 当前 template:"${this.pusher.template}"`
        )
      }
    ...
    setViewZIndex: function(params) {
      ...
      if (this.pusher.template !== 'custom') {
        console.warn(
          `如需使用setViewZIndex方法，请设置template:"custom", 当前 template:"${this.pusher.template}"`
        )
      }
    ...
    setPlayerConfigFun(params) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (user && user.streams[streamType]) {
          user.streams[streamType] = Object.assign(
            user.streams[streamType],
            config
    ...
    getPushUrlFun: function(rtcConfig) {
      ...
      if (ENV.IS_TRTC) {
        ...
        return new Promise((resolve, reject) => {
          ...
          rtcConfig.scene =
            !rtcConfig.scene || rtcConfig.scene === 'rtc' ? 'videocall' : 'live'
        ...
        wx.request({
          ...
          success: res => {
            ...
            let pushUrl =
              ...
              userID +
              '&roomsig=' +
              encodeURIComponent(roomSig) // TODO 需要重新整理的逻辑
