<template>
  <view class="u-field" :class="{ 'border-t': borderTop, 'border-b': borderBottom }">
    <view class="u-field-inner" :class="[type == 'textarea' ? 'u-textarea-inner' : '', 'u-label-postion-' + labelPosition]">
      ...
      >
        <view class="u-icon-wrap" v-if="icon">
          <van-icons size="32" :custom-style="iconStyle" :type="icon" :color="iconColor" class="u-icon" />
        </view>
        <slot name="icon"></slot>
        <text class="u-label-text" :class="[this.$slots.icon || icon ? 'u-label-left-gap' : '']">{{ label }}</text>
      </view>
      <view class="fild-body">
        <view class="flex-1 flex" :style="[inputWrapStyle]">
          <textarea v-if="type == 'textarea'" class="flex-1 u-textarea-class" :style="[fieldStyle]" :value="value" :placeholder="placeholder" :placeholderStyle="placeholderStyle" :disabled="disabled" :maxlength="inputMaxlength" :focus="focus" :autoHeight="autoHeight" :fixed="fixed" @input="onInput" @blur="onBlur" @focus="onFocus" @confirm="onConfirm" @tap="fieldClick" />
          <input v-else :style="[fieldStyle]" :type="type" class="flex-1 u-field__input-wrap" :value="value" :password="password || this.type === 'password'" :placeholder="placeholder" :placeholderStyle="placeholderStyle" :disabled="disabled" :maxlength="inputMaxlength" :focus="focus" :confirmType="confirmType" @focus="onFocus" @blur="onBlur" @input="onInput" @confirm="onConfirm" @tap="fieldClick" />
        </view>
        <van-icons :size="clearSize" v-if="clearable && value != '' && focused" type="close" color="#c0c4cc" class="u-clear-icon" @click="onClear" />
        <view class="u-button-wrap"><slot name="right"/></view>
        <van-icons v-if="rightIcon" @click="rightIconClick" :type="rightIcon" color="#c0c4cc" :style="[rightIconStyle]" size="26" class="u-arror-right" />
      </view>
    </view>
    <view
