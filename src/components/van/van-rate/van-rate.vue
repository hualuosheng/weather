<template>
  <view class="u-rate" :id="elId" @touchmove.stop.prevent="touchMove">
    <view class="u-star-wrap" v-for="(item, index) in count" :key="index" :class="[elClass]">
      <van-icons
        :type="activeIndex > index ? elActiveIcon : inactiveIcon"
        @click="click(index + 1, $event)"
        :color="activeIndex > index ? elActiveColor : inactiveColor"
        :size="size"
        :style="{ padding: `0 ${gutter / 2 + 'rpx'}` }"
      />
    </view>
  </view>
</template>
 ...
 * @example <u-rate :count="count" v-moduel="value"></u-rate>
 */
...
export default {
  ...
  computed: {
    ...
    //   }
    // },
    // :show-decimal-icon="showDecimalIcon(index)" :percent="decimal"
    elActiveIcon() {
  ...
  methods: {
    ...
    //   console.log(parseInt(this.activeIndex) === index)
